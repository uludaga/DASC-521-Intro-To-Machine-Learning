...
def regressogram(x_query, x_train, y_train, left_borders, right_borders):
    ...
    y_hat = np.asarray([
        np.sum((np.digitize(x_query[i], bins=left_borders, right=True) == np.digitize(x_train, bins=left_borders, right=True)) * y_train) /
        np.sum(np.digitize(x_query[i], bins=left_borders, right=True) == np.digitize(x_train, bins=left_borders, right=True))
...
def running_mean_smoother(x_query, x_train, y_train, bin_width):
    # your implementation starts below
    y_hat = np.asarray([
        np.sum([(-0.5 <= (x - x_train[i]) / bin_width < 0.5) * y_train[i] for i in range(len(x_train))]) /
...
def kernel_smoother(x_query, x_train, y_train, bin_width):
    # your implementation starts below
    y_hat = np.asarray([
        np.sum([1 / np.sqrt(2 * np.pi) * np.exp(-((x - x_train[i]) / bin_width)**2 / 2) * y_train[i] for i in range(len(x_train))]) /
